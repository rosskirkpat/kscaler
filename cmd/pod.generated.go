...
var (
	podCmd = &cobra.Command{
		Use:   "pod [-n 1000] [--kubeconfig /path/to/kube/config]",
		Short: "scale pod",
		Run:   scalePod,
		Hidden: true,
	}
...
func scalePod(cmd *cobra.Command, args []string) {
	var k KScaleConfig
	...
	//cmd.PersistentFlags().StringVarP(&number, "number", "n", "", "Number of pod to create")
	//cmd.Flags().StringVarP(&number, "number", "n", "", "Number of pod to create")
	...
	logrus.Printf("[scalePod] attempting to create (%v) resources of type (%v)\n", viper.GetInt("request"), pod)
	
	err := Parser()
	if err != nil {
		cmd.Println("")
		cmd.Println("[scalePod] error encountered")
