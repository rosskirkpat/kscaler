...
var (
	roleBindingCmd = &cobra.Command{
		Use:   "roleBinding [-n 1000] [--kubeconfig /path/to/kube/config]",
		Short: "scale roleBinding",
		Run:   scaleRoleBinding,
		Hidden: true,
	}
	proleBinding string
)
...
func scaleRoleBinding(cmd *cobra.Command, args []string) {
	var k KScaleConfig
	...
	//cmd.PersistentFlags().StringVarP(&number, "number", "n", "", "Number of roleBinding to create")
	//cmd.Flags().StringVarP(&number, "number", "n", "", "Number of roleBinding to create")
	...
	logrus.Printf("[scaleRoleBinding] attempting to create (%v) resources of type (%v)\n", viper.GetInt("request"), roleBinding)
	
	err := Parser()
	if err != nil {
		cmd.Println("")
		cmd.Println("[scaleRoleBinding] error encountered")
