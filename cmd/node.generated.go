...
var (
	nodeCmd = &cobra.Command{
		Use:   "node [-n 1000] [--kubeconfig /path/to/kube/config]",
		Short: "scale node",
		Run:   scaleNode,
		Hidden: true,
	}
...
func scaleNode(cmd *cobra.Command, args []string) {
	var k KScaleConfig
	...
	//cmd.PersistentFlags().StringVarP(&number, "number", "n", "", "Number of node to create")
	//cmd.Flags().StringVarP(&number, "number", "n", "", "Number of node to create")
	...
	logrus.Printf("[scaleNode] attempting to create (%v) resources of type (%v)\n", viper.GetInt("request"), node)
	
	err := Parser()
	if err != nil {
		cmd.Println("")
		cmd.Println("[scaleNode] error encountered")
